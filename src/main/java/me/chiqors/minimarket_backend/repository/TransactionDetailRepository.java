...
public interface TransactionDetailRepository extends JpaRepository<TransactionDetail, Long> {
    ...
    @Query(value = "SELECT P.sku_code, P.name, pc.NAME AS category, COUNT(td.id) AS total_purchased, P.price, P.stock FROM transaction_details td JOIN products P ON td.product_id = P.id JOIN product_categories pc ON P.product_category_id = pc.ID WHERE td.transaction_id IN ( SELECT td.transaction_id FROM transaction_details td JOIN products P ON td.product_id = P.id WHERE P.sku_code = :skuCode ) AND P.sku_code != :skuCode GROUP BY P.sku_code, P.name, pc.NAME, P.price, P.stock ORDER BY total_purchased DESC LIMIT 3", nativeQuery = true)
